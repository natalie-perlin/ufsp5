...
#modified by yanyun Liu on Dec2020

rt_45d() {
  local sy=$(echo ${DATE_45D} | cut -c 1-4)
  local sm=$(echo ${DATE_45D} | cut -c 5-6)
  local sd=$(echo ${DATE_45D} | cut -c 7-8)
  local new_test_name="tests/${TEST_NAME}_${DATE_45D}"
  ...
  if [[ $TEST_NAME =~ cold$ ]]; then
    ...
    if [[ $TEST_NAME =~ ww3 ]]; then
      sed -i -e "s/\(export CNTL_DIR\)/\1='RT-Baselines_cold_bmwav_cmeps_${DATE_45D}'/" $new_test_name
      sed -i -e "s/\(export CNTLMED_DIR\)/\1='MEDIATOR_bmwav_cmeps_${DATE_45D}'/" $new_test_name
    else
#      sed -i -e "s/\(export CNTL_DIR\)/\1='RT-Baselines_cold_bm_cmeps_${DATE_45D}'/" $new_test_name
#      sed -i -e "s/\(export CNTLMED_DIR\)/\1='MEDIATOR_bm_cmeps_${DATE_45D}'/" $new_test_name
      sed -i -e "s/\(export CNTL_DIR\)/\1='RT-Baselines_cold_ccpp_cmeps_${DATE_45D}'/" $new_test_name
      sed -i -e "s/\(export CNTLMED_DIR\)/\1='MEDIATOR_ccpp_cmeps_${DATE_45D}'/" $new_test_name
    fi
  elif [[ $TEST_NAME =~ 45d$ ]]; then
    sed -i -e "s/\(export SYEAR\)/\1=\"$sy\"/" $new_test_name
    sed -i -e "s/\(export SMONTH\)/\1=\"$sm\"/" $new_test_name
    sed -i -e "s/\(export SDAY\)/\1=\"$sd\"/" $new_test_name
    DEP_RUN=${DEP_RUN}_${DATE_45D}
    if [[ $TEST_NAME =~ ww3 ]]; then
      sed -i -e "s/\(export MED_restart_data\)/\1='MEDIATOR_bmwav_cmeps_${DATE_45D}'/" $new_test_name
    else
     sed -i -e "s/\(export MED_restart_data\)/\1='MEDIATOR_ccpp_cmeps_${DATE_45D}'/" $new_test_name
#      sed -i -e "s/\(export MED_restart_data\)/\1='MEDIATOR_bm_cmeps_${DATE_45D}'/" $new_test_name
    fi
  fi
...
while read -r line; do
  ...
  if [[ $line == COMPILE* ]] ; then
    ...
    DATE_45D=$( echo $line | cut -d'|' -f7 | sed -e 's/^ *//' -e 's/ *$//')
#    DATE_45D=$(  echo $line | cut -d'|' -f6 | sed -e 's/^ *//' -e 's/ *$//')
...
#  [[ ${TEST_45D} == true ]] && rm -f tests/cpld_*cmeps*_20*
fi
