# 1 "/lustre/f2/dev/ncep/JieShun.Zhu/ufsp5/ufs-s2s-model_zbotC5/FV3/ccpp/physics/physics/radlw_main.f"
!>  \file radlw_main.f
...
!> This module contains the 1-compliant NCEP's modifications of the
...
!ncep model use
     ...
     &                           *amdw/(f_one-qlyr(iplon,k1)))          
            o3vmr (k)= max(f_zero,olyr(iplon,k1)*amdo3)                 
...
!  --- ...  tem0 is the molecular weight of moist air
            ...
            colamt(k,2) = max(temcol(k), coldry(k)*gasvmr_co2(iplon,k1))
...
!!    coldry (scaled to 1.0e-20).
          ...
          if (ilwrgas > 0) then
            do k = 1, nlay
              k1 = nlp1 - k
              colamt(k,4)=max(temcol(k), coldry(k)*gasvmr_n2o(iplon,k1))
              colamt(k,5)=max(temcol(k), coldry(k)*gasvmr_ch4(iplon,k1))
              colamt(k,6)=max(f_zero,    coldry(k)*gasvmr_o2(iplon,k1)) 
              colamt(k,7)=max(f_zero,    coldry(k)*gasvmr_co(iplon,k1)) 

              wx(k,1) = max( f_zero, coldry(k)*gasvmr_ccl4(iplon,k1) )  
              wx(k,2) = max( f_zero, coldry(k)*gasvmr_cfc11(iplon,k1) ) 
              wx(k,3) = max( f_zero, coldry(k)*gasvmr_cfc12(iplon,k1) ) 
              wx(k,4) = max( f_zero, coldry(k)*gasvmr_cfc22(iplon,k1) ) 
            enddo
          else
            do k = 1, nlay
...
!ncep model use
     ...
     &                           *amdw/(f_one-qlyr(iplon,k)))           
            o3vmr (k)= max(f_zero,olyr(iplon,k)*amdo3)                  
...
!           to molec/cm2 based on coldry (scaled to 1.0e-20)
          ...
          if (ilwrgas > 0) then
            do k = 1, nlay
              colamt(k,4)=max(temcol(k), coldry(k)*gasvmr_n2o(iplon,k)) 
              colamt(k,5)=max(temcol(k), coldry(k)*gasvmr_ch4(iplon,k)) 
              colamt(k,6)=max(f_zero,    coldry(k)*gasvmr_o2(iplon,k))  
              colamt(k,7)=max(f_zero,    coldry(k)*gasvmr_co(iplon,k))  

              wx(k,1) = max( f_zero, coldry(k)*gasvmr_ccl4(iplon,k) )   
              wx(k,2) = max( f_zero, coldry(k)*gasvmr_cfc11(iplon,k) )  
              wx(k,3) = max( f_zero, coldry(k)*gasvmr_cfc12(iplon,k) )  
              wx(k,4) = max( f_zero, coldry(k)*gasvmr_cfc22(iplon,k) )  
            enddo
          else
            do k = 1, nlay
              colamt(k,4) = f_zero     ! n2o
