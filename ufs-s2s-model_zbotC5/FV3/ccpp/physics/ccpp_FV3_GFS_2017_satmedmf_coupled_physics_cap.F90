...
!!
!
module ccpp_FV3_GFS_2017_satmedmf_coupled_physics_cap

   use lsm_noah, only: lsm_noah_init
   use satmedmfvdif, only: satmedmfvdif_init
   use cires_ugwp, only: cires_ugwp_init
   use ozphys, only: ozphys_init
   ...
   use satmedmfvdif, only: satmedmfvdif_run
   ...
   private
   public :: FV3_GFS_2017_satmedmf_coupled_physics_init_cap, &
             FV3_GFS_2017_satmedmf_coupled_physics_run_cap, &
             FV3_GFS_2017_satmedmf_coupled_physics_finalize_cap

   logical, save :: initialized = .false.

   contains

   function FV3_GFS_2017_satmedmf_coupled_physics_init_cap(con_p0,cdata,GFS_Control) result(ierr)
      ...
      call satmedmfvdif_init(isatmedmf=GFS_Control%isatmedmf,isatmedmf_vdif=GFS_Control%isatmedmf_vdif, &
                  errmsg=cdata%errmsg,errflg=cdata%errflg)



      if (cdata%errflg/=0) then
        cdata%errmsg = "An error occured in satmedmfvdif_init: " // trim(cdata%errmsg)
        ierr=cdata%errflg
        return
      end if
      ...
      call ozphys_init(oz_phys=GFS_Control%oz_phys,errmsg=cdata%errmsg,errflg=cdata%errflg)



      if (cdata%errflg/=0) then
        cdata%errmsg = "An error occured in ozphys_init: " // trim(cdata%errmsg)
        ierr=cdata%errflg
        return
      end if
   ...
   end function FV3_GFS_2017_satmedmf_coupled_physics_init_cap

   function FV3_GFS_2017_satmedmf_coupled_physics_run_cap(con_sbc,con_pi,GFS_Control,GFS_Data,con_cliq,con_epsm1,con_hvap,con_hfus, &
                  GFS_Interstitial,con_rd,con_cvap,con_rv,con_g,con_cp,huge,con_t0c,con_fvirt, &
                  con_tice,cdata,con_eps,cimin) result(ierr)
      ...
      call satmedmfvdif_run(im=GFS_Control%blksz(cdata%blk_no),km=GFS_Control%levs,ntrac=GFS_Interstitial(cdata%thrd_no)%nvdiff, &
                  ntcw=GFS_Control%ntcw,ntiw=GFS_Interstitial(cdata%thrd_no)%ntiwx,ntke=GFS_Interstitial(cdata%thrd_no)%ntkev, &
                  grav=con_g,rd=con_rd,cp=con_cp,rv=con_rv,hvap=con_hvap,hfus=con_hfus,fv=con_fvirt, &
                  eps=con_eps,epsm1=con_epsm1,dv=GFS_Interstitial(cdata%thrd_no)%dvdt,du=GFS_Interstitial(cdata%thrd_no)%dudt, &
                  tdt=GFS_Interstitial(cdata%thrd_no)%dtdt,rtg=GFS_Interstitial(cdata%thrd_no)%dvdftra, &
                  ...
                  xmu=GFS_Interstitial(cdata%thrd_no)%xmu,garea=GFS_Data(cdata%blk_no)%Grid%area, &
                  ...
                  heat=GFS_Interstitial(cdata%thrd_no)%hflxq,evap=GFS_Interstitial(cdata%thrd_no)%evapq, &
                  ...
                  hpbl=GFS_Data(cdata%blk_no)%Tbd%hpbl,kinver=GFS_Interstitial(cdata%thrd_no)%kinver, &
                  xkzm_m=GFS_Control%xkzm_m,xkzm_h=GFS_Control%xkzm_h,xkzm_s=GFS_Control%xkzm_s, &
                  dt3dt_PBL=GFS_Data(cdata%blk_no)%Intdiag%dt3dt(:,:,3),du3dt_PBL=GFS_Data(cdata%blk_no)%Intdiag%du3dt(:,:,1), &
                  dv3dt_PBL=GFS_Data(cdata%blk_no)%Intdiag%dv3dt(:,:,1),dq3dt_PBL=GFS_Data(cdata%blk_no)%Intdiag%dq3dt(:,:,1), &
                  do3dt_PBL=GFS_Data(cdata%blk_no)%Intdiag%dq3dt(:,:,5),ldiag3d=GFS_Control%ldiag3d, &
                  qdiag3d=GFS_Control%qdiag3d,errmsg=cdata%errmsg,errflg=cdata%errflg)



      if (cdata%errflg/=0) then
        cdata%errmsg = "An error occured in satmedmfvdif_run: " // trim(cdata%errmsg)
        ierr=cdata%errflg
        return
      end if
      ...
      call ozphys_run(im=GFS_Control%blksz(cdata%blk_no),levs=GFS_Control%levs,ko3=GFS_Interstitial(cdata%thrd_no)%levozp, &
      ...
      if (cdata%errflg/=0) then
        cdata%errmsg = "An error occured in ozphys_run: " // trim(cdata%errmsg)
        ierr=cdata%errflg
        return
      end if
      ...
      call zhaocarr_gscond_run(im=GFS_Control%blksz(cdata%blk_no),km=GFS_Control%levs,dt=GFS_Control%dtp, &
                  dtf=GFS_Control%dtf,prsl=GFS_Data(cdata%blk_no)%Statein%prsl,ps=GFS_Data(cdata%blk_no)%Statein%pgr, &
                  q=GFS_Data(cdata%blk_no)%Stateout%gq0(:,:,GFS_Control%ntqv),clw1=GFS_Interstitial(cdata%thrd_no)%clw(:,:,1), &
                  clw2=GFS_Interstitial(cdata%thrd_no)%clw(:,:,2),cwm=GFS_Data(cdata%blk_no)%Stateout%gq0(:,:,GFS_Control%ntcw), &
                  t=GFS_Data(cdata%blk_no)%Stateout%gt0,tp=GFS_Data(cdata%blk_no)%Tbd%phy_f3d(:,:,1), &
                  qp=GFS_Data(cdata%blk_no)%Tbd%phy_f3d(:,:,2),psp=GFS_Data(cdata%blk_no)%Tbd%phy_f2d(:,1), &
                  tp1=GFS_Data(cdata%blk_no)%Tbd%phy_f3d(:,:,3),qp1=GFS_Data(cdata%blk_no)%Tbd%phy_f3d(:,:,4), &
                  psp1=GFS_Data(cdata%blk_no)%Tbd%phy_f2d(:,2),u=GFS_Interstitial(cdata%thrd_no)%rhc, &
                  lprnt=GFS_Control%lprnt,ipr=GFS_Interstitial(cdata%thrd_no)%ipr,errmsg=cdata%errmsg, &
                  errflg=cdata%errflg)



      if (cdata%errflg/=0) then
        cdata%errmsg = "An error occured in zhaocarr_gscond_run: " // trim(cdata%errmsg)
        ierr=cdata%errflg
        return
      end if
      ...
      call zhaocarr_precpd_run(im=GFS_Control%blksz(cdata%blk_no),km=GFS_Control%levs,dt=GFS_Control%dtp, &
                  del=GFS_Interstitial(cdata%thrd_no)%del,prsl=GFS_Data(cdata%blk_no)%Statein%prsl, &
                  ...
                  sr=GFS_Data(cdata%blk_no)%Intdiag%sr,rainp=GFS_Interstitial(cdata%thrd_no)%rainp, &
      ...
      if (cdata%errflg/=0) then
        cdata%errmsg = "An error occured in zhaocarr_precpd_run: " // trim(cdata%errmsg)
        ierr=cdata%errflg
        return
      end if
   ...
   end function FV3_GFS_2017_satmedmf_coupled_physics_run_cap

   function FV3_GFS_2017_satmedmf_coupled_physics_finalize_cap(cdata) result(ierr)
   ...
   end function FV3_GFS_2017_satmedmf_coupled_physics_finalize_cap

end module ccpp_FV3_GFS_2017_satmedmf_coupled_physics_cap
